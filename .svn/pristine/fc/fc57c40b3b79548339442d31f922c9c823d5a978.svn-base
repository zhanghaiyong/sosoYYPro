...
-(void)setFilterIndexPath:(NSIndexPath *)indexPath andTitleAry:(NSArray *)titleAry productDict:(NSDictionary *)dict{
    ...
    switch (indexPath.section) {
        case 0:
            if ([dict[@"producttype"] intValue] == indexPath.row) {
            ...
            break;
        case 1:
            if ([dict[@"cuxiao"] intValue] == indexPath.row) {
                self.titleLab.textColor = [UIColor fromHexValue:0xea5413 alpha:1];
                self.layer.borderColor = [UIColor fromHexValue:0xea5413 alpha:1].CGColor;
                self.layer.borderWidth = 1.0f;
            }else{
            ...
            break;
        case 2:
            if ([dict[@"GrossMarginRange"] intValue] == indexPath.row) {
                self.titleLab.textColor = [UIColor fromHexValue:0xea5413 alpha:1];
                self.layer.borderColor = [UIColor fromHexValue:0xea5413 alpha:1].CGColor;
                self.layer.borderWidth = 1.0f;
            }else{
            ...
            break;
        case 3:
            if ([dict[@"PriceRange"] intValue] == indexPath.row) {
                self.titleLab.textColor = [UIColor fromHexValue:0xea5413 alpha:1];
                self.layer.borderColor = [UIColor fromHexValue:0xea5413 alpha:1].CGColor;
                self.layer.borderWidth = 1.0f;
            }else{
