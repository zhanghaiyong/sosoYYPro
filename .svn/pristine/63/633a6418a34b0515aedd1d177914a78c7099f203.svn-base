...
@property(strong,nonatomic)UILabel *showContinuePayLab;
...
- (void)viewDidLoad {
    ...
    _showContinuePayLab = [[UILabel alloc]initWithFrame:CGRectMake(0, kNavBarHeight, kScreenWidth, 0)];
    _showContinuePayLab.backgroundColor = [UIColor redColor];
    _showContinuePayLab.textColor = [UIColor whiteColor];
    _showContinuePayLab.numberOfLines = 0;
    _showContinuePayLab.font = [UIFont systemFontOfSize:13];
    _showContinuePayLab.text = @" 请完成剩下店铺的支付，如果您选择取消，之前白条支付订单依然有效";
    [self.view addSubview:_showContinuePayLab];
    
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, _showContinuePayLab.bottom, kScreenWidth, kScreenHeight - kNavBarHeight-_showContinuePayLab.height)style:UITableViewStyleGrouped];
...
- (void)setMasterOid:(NSString *)masterOid {

    _masterOid = masterOid;
        ...
        case 1:{
            ...
            //白条支付
                ...
                if ([_result[@"OrderAmount_NoteBlank"] doubleValue] < 500) {
                    ...
                    [ZHProgressHUD showInfoWithText:@"可使用白条支付的订单不足500,无法使用"];
                    
                }else {
                
                    if ([_result[@"OrderAmount_NoteBlank"] doubleValue] > [_result[@"UsableCreditLimit"] doubleValue]) {
                        
                        [ZHProgressHUD showInfoWithText:@"您的白条额度不足,本次订单无法使用"];
                        ...
                        if ([_result[@"WaitPayMoney"] doubleValue] == [_result[@"OrderAmount_NoteBlank"] doubleValue]) {
                            ...
                            MixturePayGoodListViewController *MixturePayVC = [[MixturePayGoodListViewController alloc]init];
                            MixturePayVC.result = _result;
                            [self.navigationController pushViewController:MixturePayVC animated:YES];
                        }
                    }
                }
            }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == 0) {
        ...
        _methodPaymentHeadView.frame = CGRectMake(0, 0, kScreenWidth, 60);
        [_methodPaymentHeadView setMethodPayment:_result];
        [headView addSubview:_methodPaymentHeadView];
        
        titel.frame = CGRectMake(10, _methodPaymentHeadView.bottom, kScreenWidth - 20, 30);
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (self.isShowAlert) {
        _showContinuePayLab.frame = CGRectMake(0, kNavBarHeight, kScreenWidth, 40);
        _tableView.frame = CGRectMake(0, _showContinuePayLab.bottom, kScreenWidth, kScreenHeight - kNavBarHeight-_showContinuePayLab.height);
    }else {
        _showContinuePayLab.frame = CGRectMake(0, kNavBarHeight, kScreenWidth, 0);
        _tableView.frame = CGRectMake(0, _showContinuePayLab.bottom, kScreenWidth, kScreenHeight - kNavBarHeight-_showContinuePayLab.height);
    }
