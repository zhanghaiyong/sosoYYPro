...
//            [detailsVC setHidesBottomBarWhenPushed:YES];
    ...
    //编辑税号
    self.jsContext[@"edittaxesnum"]=^(){
        
        NSArray *arry = [JSContext currentArguments];
        DutyCodeViewController *DutyCodeVC = [DutyCodeViewController new];
        if (arry.count > 0) {
            DutyCodeVC.dutyStr = [NSString stringWithFormat:@"%@",arry[0]];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakself tabBarControllerHidden];
            [weakself.navigationController pushViewController:DutyCodeVC animated:YES];
        });
    };
}
