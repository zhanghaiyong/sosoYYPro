...
@interface STWisdomAddListBtnView (){
    NSInteger typeTag;
    BOOL isSeleceted;
    NSInteger isFrist;
    NSInteger oldtag;
...
@implementation STWisdomAddListBtnView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        isSeleceted = YES;
        isFrist = kSTListBtntag + 3;
        [self setSubBtn];
    }
    return self;
}
-(void)setSubBtn{
    _labelAry = @[@"销量",@"价格  ",@"毛利率 ",@"  筛选"];
    ...
    for (int i = 0; i < _labelAry.count; i++) {
        ...
        if (i < 3) {
            ...
            [Btn addTarget:self action:@selector(selectBtnA:) forControlEvents:UIControlEventTouchUpInside];
        }
...
-(void)selectBtn:(UIButton *)btn{
    ...
    if (_delegate && [ _delegate respondsToSelector:@selector(g_setWisdomAddListBtnTag:andSelected:)]) {
        [_delegate g_setWisdomAddListBtnTag:btn.tag - kSTListBtntag andSelected:isSeleceted];
    }
    typeTag = btn.tag;
}
-(void)selectBtnA:(UIButton *)btn{
    if (_delegate && [ _delegate respondsToSelector:@selector(g_setWisdomAddListBtnTag:andSelected:)]) {
        [_delegate g_setWisdomAddListBtnTag:btn.tag - kSTListBtntag andSelected:isSeleceted];
    }
    typeTag = btn.tag;
